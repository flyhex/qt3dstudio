...
//==============================================================================
const QT3DSF32 M_PHI = 3.14159265358979323846f;

/* epsilon surrounding for near zero values */
const QT3DSF32 EQN_EPS = 1e-9f;
...
#ifndef CUBEROOT
#define CUBEROOT(x)                                                                                \
    ((x) > 0.0 ? static_cast<QT3DSF32>(pow((x), 1.0f / 3.0f))                                         \
               : ((x) < 0.0 ? -static_cast<QT3DSF32>(pow(-(x), 1.0f / 3.0f)) : 0.0f))
#endif
 ...
 */
QT3DSI32 CCubicRoots::SolveQuadric(QT3DSF32 inConstants[3], QT3DSF32 outSolutions[2])
{
#ifdef _PERF_LOG
    PerfLogMathEvent1 thePerfLog(DATALOGGER_CUBICROOT);
#endif
    QT3DSF32 theP, theQ, theD;
    QT3DSI32 theResult = 0;
    ...
    if (ISZERO(theD)) {
        ...
        QT3DSF32 theSquareRootD = sqrtf(theD);
 ...
 *	@return the number of solutions for the equation
 */
QT3DSI32 CCubicRoots::SolveCubic(QT3DSF32 inConstants[4], QT3DSF32 outSolutions[3])
{
#ifdef _PERF_LOG
    PerfLogMathEvent1 thePerfLog(DATALOGGER_CUBICROOT);
#endif
    ...
    QT3DSI32 theResult;
    QT3DSF32 theSubstitute;
    QT3DSF32 theVariableA, theVariableB, theVariableC;
    QT3DSF32 theASquared, theVariableP, theVariableQ;
    QT3DSF32 thePCubed, theVariableD;
    ...
    if (ISZERO(theVariableD)) {
        ...
        {
            QT3DSF32 theVariableU = CUBEROOT(-theVariableQ);
    ...
    {
        QT3DSF32 thePhi = 1.0f / 3.0f * static_cast<QT3DSF32>(acos(-theVariableQ / sqrtf(-thePCubed)));
        QT3DSF32 theVariableT = 2.0f * sqrtf(-theVariableP);

        outSolutions[0] = theVariableT * static_cast<QT3DSF32>(cos(thePhi));
        outSolutions[1] = -theVariableT * static_cast<QT3DSF32>(cos(thePhi + M_PHI / 3.0f));
        outSolutions[2] = -theVariableT * static_cast<QT3DSF32>(cos(thePhi - M_PHI / 3.0f));
        theResult = 3;
    } else /* one real solution */
    {
        QT3DSF32 theSquareRootD = sqrtf(theVariableD);
        QT3DSF32 theVariableU = CUBEROOT(theSquareRootD - theVariableQ);
        QT3DSF32 theVariableV = -CUBEROOT(theSquareRootD + theVariableQ);
    ...
    for (QT3DSI32 theIndex = 0; theIndex < theResult; ++theIndex)
